...
# NB1 vegleges tabella-tortenet potlasa: a nb1_final_tables_historical eddig
# csak 2022-23-at es 2023-24-et tartalmazta (soha nem keszult hozza folytato
# scraper, szemben a nb1_squad_values_historical-lel, aminek van
# refresh_nb1_2627_squad_values.R-je). Ez a script a 2024-25 es 2025-26
# (mindketto mar lezart szezon) tabellajat potolja, ugyanazzal a HTML-parsing
# mintaval, amit a scrape_tm_historical_nb2.R mar hasznal az NB2-re.
...
SEASONS <- c(2024, 2025)
SZEZON_LABELS <- c("2024" = "2024-25", "2025" = "2025-26")

# team_name_mapping.csv: tm_name -> canonical_name (ugyanaz a lista, amit a
# Next.js oldal src/lib/teamNameMap.ts-e is hasznal).
...
scrape_season_table <- function(start_year) {
  table_url <- paste0(
    "https://www.transfermarkt.com/nemzeti-bajnoksag/tabelle/wettbewerb/UNG1?saison_id=",
    start_year
  )
...
for (start_year in SEASONS) {
  ...
  if (!is.null(res) && nrow(res) > 0) {
    ...
    dbExecute(con, "DELETE FROM nb1_final_tables_historical WHERE szezon = ?", params = list(label))
    dbWriteTable(con, "nb1_final_tables_historical", res, append = TRUE)
  } else {
